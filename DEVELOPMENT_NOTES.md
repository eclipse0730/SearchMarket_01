...
- `kospi-all`: FDR/KRX가 가능하면 KOSPI 전체 종목을 수집하는 선택형 시장입니다. 실패 시 정적 메타데이터 fallback을 사용합니다.
- `kosdaq-all`: FDR/KRX가 가능하면 KOSDAQ 전체 종목을 수집하는 선택형 시장입니다. 실패 시 정적 메타데이터 fallback을 사용합니다.
...
- 한국 전체 유니버스: 기본 `kospi`/`kosdaq`은 대형주 중심을 유지하고, 전체 수집은 `kospi-all`/`kosdaq-all` 별도 시장으로 제공합니다.
