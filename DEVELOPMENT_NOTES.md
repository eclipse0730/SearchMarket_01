...
이 문서는 프로젝트를 함께 개발하면서 계속 갱신하는 개발/운영 노트입니다. 자동 생성되는 `Analysis_*.md` 리포트와 구분하기 위해, 기능 개발 판단과 코드 구조 이해에 필요한 내용을 이 파일에 모읍니다.
...
- `us`: 미국 주식. 정적 NASDAQ 100 메타데이터와 S&P 500 라이브/캐시 목록을 결합합니다.
...
- `Search.py`: CLI 진입점입니다. `--market`, `--stage`, `--date`, `--force`, `--workers`, `--setup-scheduler` 옵션을 처리합니다.
...
- `market_scanner/pipeline.py`: yfinance 기반 스캔, RSI/추세/점수 계산, Markdown/HTML 리포트 생성을 담당합니다.
...
- KOSPI, KOSDAQ: 렌더링 단계에서 종목 수만큼 외부 검색 요청이 나가는 것을 막기 위해 `kr.investing.com/search?q=종목코드` 형태의 검색 링크를 사용합니다.
...
- 현재 동작: 캐시가 있으면 뉴스 항목을 표시하고, 캐시가 없으면 수집 필요 안내를 표시합니다.
...
```powershell
.\.venv\Scripts\python.exe Search.py --market us
```

단계별 흐름:

1. `scan`: 시장 유니버스를 불러오고 yfinance에서 가격 히스토리와 ticker info를 가져와 `data/`에 CSV를 저장합니다.
2. `analyze`: CSV를 기반으로 요약 Markdown 리포트를 생성합니다.
3. `translate`: US 시장 전용으로 이름/설명 일부를 번역하고 sector alias를 적용합니다.
4. `render`: CSV와 Markdown을 HTML 리포트로 렌더링합니다.
5. `all`: 위 단계를 순서대로 실행합니다.
...
- 기타 시장: `data/Data_{MarketName}_YYYYMMDD.csv`, `Analysis_{MarketName}_YYYYMMDD.md`, `Report_{MarketName}_YYYYMMDD.html`

CSV 경로는 `data/`가 canonical 위치입니다. 과거 루트 `Data_*.csv`는 읽기 fallback으로만 지원합니다.
...
현재 종합 점수는 추세, RSI, 목표가 대비 upside, PER, 52주 고점 대비 위치, 거래량 비율, 이동평균 근접 개수를 가중합합니다.

추세 점수는 `calc_trend`에서 0~5점으로 계산합니다. 기준은 현재가가 단기 MA 위에 있는지, 단기 MA가 중기/장기 MA보다 위인지, 단기/중기 MA의 20거래일 전 대비 기울기가 상승인지입니다.

## Site Build Flow

GitHub Pages 배포는 `python -m market_scanner.site_builder`로 `site/` 폴더를 재생성합니다.

사이트 생성 방식:

- `data/`의 최신 `Data_*.csv`를 기준으로 사이트 페이지를 생성합니다. 과거 루트 CSV가 있으면 fallback으로 읽을 수 있습니다.
- 기존 `Report_*.html`이 있으면 사이트 shell 안에 삽입하고, 없으면 CSV/Markdown을 사용해 페이지를 재렌더링합니다.
- US 전체 데이터에서 NASDAQ 100, S&P 500, Dow 30 페이지를 파생 생성합니다.
- 메인 페이지에는 Market Regime, Equity Breadth, MA Concentration, RSI Temperature, 시장별 스냅샷, 섹터 리더십을 표시합니다.
- 상세 페이지는 좌측 종목 리스트와 우측 인사이트 패널 구조입니다. 우측에는 Sector Heatmap, Fear & Volatility, Setup Buckets, MA Distance vs RSI Scatter를 표시합니다.
...
- `.github/workflows/deploy-pages.yml`: 데이터/리포트/코드 변경 후 Pages 사이트를 빌드하고 배포합니다.
...
- Manual override: `market_scanner/assets/sp500_members_manual.json`
...
캐시가 45일보다 오래되면 콘솔에 stale warning을 출력합니다. 수동 override는 `add`와 `remove` 배열로 관리하며, 외부 소스가 일시적으로 오래됐거나 잘못됐을 때만 사용합니다.
...
- `.\.venv\Scripts\python.exe Search.py --market us --stage render --date 20260428` 통과. 생성된 `Report_20260428.html`의 US 티커 링크가 `kr.investing.com`으로 출력되는 것을 확인함.
...
- 상세페이지 v2 렌더 확인: `Report_Kospi_20260427.html`, `site/kospi/index.html`, `site/dow30/index.html`에 Sector Heatmap, Setup Buckets, MA Distance Scatter, Fear & Volatility 패널 반영됨.
- 상세페이지 UX 개선: KOSPI/KOSDAQ 티커 링크도 `kr.investing.com` 검색 링크로 연결합니다. Sector Heatmap은 섹터 평균 등락률 기준으로 강세가 앞쪽, 약세가 뒤쪽으로 정렬됩니다. MA Distance vs RSI에는 지표 설명과 한줄 해석을 추가했고 scatter point hit radius를 키웠습니다.
...
- `git status`에 루트 `Report_*.html` 삭제가 표시되어 있습니다. 기존 사용자 변경일 수 있으므로 함부로 복구하거나 커밋하지 않습니다.

## Known Risks And Improvement Areas

- S&P 500 유니버스 갱신은 네트워크 실패 시 캐시 품질에 의존합니다.
- S&P 500 캐시는 구조화 메타데이터와 수동 override를 지원하지만, 무료 소스 기반이므로 공식 라이선스 데이터 피드만큼 보장되지는 않습니다.
...
- 메인 페이지 고도화: 1차로 Market Regime, Equity Breadth, MA Concentration, RSI Temperature, 시장별 스냅샷, 섹터 리더십을 추가했습니다. 다음 단계는 전일 대비 변화와 신호 변화 추적입니다.
