...
대표 유니버스는 시장과 별도로 `universe_memberships`에서 관리합니다. 예를 들어 `uv run python Search.py --market us --universe sp500`, `uv run python Search.py --market kospi --universe kospi100`, `uv run python Search.py --market kospi --universe kospi200`, `uv run python Search.py --market kosdaq --universe kosdaq150`처럼 필요할 때만 멤버십 필터를 겁니다. 과거 universe형 market key는 내부 호환 경로로만 남기고 사용자 문서에서는 권장하지 않습니다.

## Main Entry Points

- `Search.py`: CLI 진입점입니다. `--market`, `--universe`, `--stage`, `--date`, `--force`, `--workers`, `--limit`, `--setup-scheduler` 옵션을 처리합니다.
...
1. `scan`: 기본값은 PostgreSQL `instruments`에서 해당 시장의 전체 활성 종목을 읽어 가격 히스토리와 지표를 수집하고 `data/`에 CSV를 저장합니다. `--universe`를 지정하면 `universe_memberships`의 현재 편입 종목만 스캔합니다. universe가 다른 시장에 속하면 오류로 중단합니다. 한국 시장은 FinanceDataReader OHLCV를 우선 사용하고, 그 외 시장은 yfinance 히스토리를 사용합니다. DB 조회가 실패하면 기존 `MARKETS[market].universe_loader()` 경로로 fallback합니다.
2. `analyze`: CSV를 기반으로 요약 Markdown 리포트를 생성합니다.
3. `translate`: US 계열 CSV의 이름/설명 일부를 번역하고 sector alias를 적용합니다.
...
2. `uv run python -m market_scanner.db init`으로 `docs/database_schema_v1.sql`을 적용하고 시장/유니버스 기준 데이터를 seed합니다. 현재 코드에 없는 market/universe 기준 row는 삭제하지 않고 `is_active = false`로 비활성화합니다. 이 흐름은 `psycopg[binary]` 의존성을 사용합니다.
3. `uv run python -m market_scanner.db refresh-master --reset`으로 기존 멤버십을 비우고 시장 유니버스 로더 기반 종목마스터와 멤버십을 다시 구성합니다. `--market kospi`는 KOSPI 전체만 갱신하고, 대표 유니버스는 `--market kospi --universe kospi100`, `--market kospi --universe kospi200`처럼 명시했을 때만 갱신합니다. `--reset`은 `universe_memberships`만 해당 범위에서 삭제 후 재생성하며, `instruments`, 가격, 지표, 스캔 결과, 뉴스, 리포트, `collection_runs` 로그는 보존합니다.
...
4. 기존 스캔 CSV를 생성하거나 재사용합니다.
5. `uv run python -m market_scanner.db load-csv --market kospi --date YYYYMMDD`로 CSV 결과를 DB에 upsert합니다.
6. `uv run python -m market_scanner.db counts`로 핵심 테이블 적재 건수를 확인합니다.
...
- KOSPI 200 필터: `data/Data_Kospi200_YYYYMMDD.csv`, `analysis/Analysis_Kospi200_YYYYMMDD.md`, `reports/Report_Kospi200_YYYYMMDD.html`
- KOSDAQ: `data/Data_Kosdaq_YYYYMMDD.csv`, `analysis/Analysis_Kosdaq_YYYYMMDD.md`, `reports/Report_Kosdaq_YYYYMMDD.html`
- S&P 500 필터: `data/Data_Sp500_YYYYMMDD.csv`, `analysis/Analysis_Sp500_YYYYMMDD.md`, `reports/Report_Sp500_YYYYMMDD.html`
...
GitHub Pages 배포는 `uv run python -m market_scanner.site_builder`로 `site/` 폴더를 재생성합니다. 로컬 실행에서는 빌드 후 `site/index.html`을 기본 브라우저로 자동으로 열고, CI/GitHub Actions에서는 자동 열기를 건너뜁니다. 로컬에서도 `--no-open`을 붙이면 브라우저를 열지 않습니다.
...
`us` 전체 시장은 NASDAQ Trader 상장 심볼 디렉터리 조회 실패에 대비해 `market_scanner/assets/us_listed_symbols_cache.json`을 사용합니다. 이 캐시는 14일보다 오래되면 stale warning을 출력하며, 스캔 대상은 ETF와 비보통주성 증권을 제외한 미국 상장 보통주 중심입니다.
...
- 미국 전체 유니버스: 기본 스캔은 `us` 시장 전체 활성 종목입니다. `nasdaq100`, `sp500` 같은 대표 그룹은 `--universe` 필터로 실행합니다. 소스는 NASDAQ Trader `nasdaqlisted.txt`/`otherlisted.txt`이며, yfinance 호환을 위해 클래스 주식 구분자는 `.`에서 `-`로 정규화합니다.
