...
3. `uv run python -m market_scanner.db refresh-master --reset`으로 기존 적재 마스터/스캔 데이터를 비우고 시장 유니버스 로더 기반 종목마스터와 멤버십을 다시 구성합니다. `--market kospi`는 KOSPI 전체만 갱신하고, 대표 유니버스는 `--market kospi --universe kospi100`, `--market kospi --universe kospi200`처럼 명시했을 때만 갱신합니다. `--market`과 `--reset`을 함께 쓰면 해당 시장의 적재 데이터만 reset하고, `--universe`와 함께 쓰면 해당 유니버스의 멤버십/산출물만 reset하며 종목마스터는 보존합니다. `collection_runs` 로그는 삭제하지 않고 누적합니다.
