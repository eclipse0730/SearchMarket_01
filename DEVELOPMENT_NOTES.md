...
- 메인페이지 v2 샘플은 `site/preview-home/index.html`에 생성합니다. 현재 메인 승격 전 시장별 상태 카드와 Today Watchlist 구성을 검토하는 임시 페이지입니다.
- 상세 페이지는 좌측 종목 리스트와 우측 인사이트 패널 구조입니다. 헤더에는 제목 아래 기준일/행 수와 `KST` 기준 갱신시간을 표시합니다. 우측에는 Sector Heatmap, Fear & Volatility, Setup Buckets, MA Distance vs RSI Scatter를 표시합니다.
