...
권장 운영 흐름은 모듈별 5단계 DB 파이프라인입니다. `Search.py`는 2~5단계를 묶어 실행하는 호환용 단축 CLI로 유지하되, 사용자 문서에서는 모듈별 명령을 기본 흐름으로 설명합니다.
...
1. 종목 수집: `uv run python -m market_scanner.storage.db refresh-master --market us`
   가격/지표를 수집하지 않고 `instruments`, `universe_memberships`, `collection_runs`만 갱신합니다. US는 `nasdaq`, `nyse`, `amex`, `nasdaq100`, `sp500` universe를, 한국 시장은 시장 전체와 대표 universe를 로더 정책에 따라 갱신합니다. `--universe`를 지정하면 해당 멤버십만 갱신합니다. `--reset`은 멤버십만 삭제 후 재생성하며 가격, 지표, 스캔 결과, 뉴스, 리포트, 실행 로그는 보존합니다. 기존 현재 멤버십과 새 수집 목록을 비교해 previous/fetched/matched/mismatch, added/removed/rank_changed 샘플, 신규/upsert instrument 샘플을 출력하고 `collection_runs.params`에 저장합니다.
2. 가격 수집: `uv run python -m market_scanner.collectors.prices fetch --market us`
   시장별 목표일 기준으로 이미 가격이 있는 종목을 SQL 단계에서 제외합니다. 기본 목표일은 한국 시장은 실행일, 비한국 시장은 KST 기준 전일이며 `--date`가 있으면 해당 날짜를 사용합니다. KOSPI/KOSDAQ은 timeout이 적용된 Naver 일봉 조회를 우선 사용하고 실패 시 yfinance 히스토리로 fallback합니다. US는 Yahoo 계열 가격 조회를 사용합니다. `fetch`와 `backfill` 모두 `--workers`로 병렬 처리하되 DB upsert는 메인 스레드에서 수행합니다.
3. 지표 계산: `uv run python -m market_scanner.analysis.indicators compute --market us`
...
현재 종합 점수는 PRD v1.1의 복합 스코어링 구조를 반영해 차트 30%, 기술지표 25%, 재무 20%, 테마 15%, 모멘텀 10%로 계산합니다. DB 호환성을 위해 가격/거래량 기반 모멘텀 점수는 `scan_results.flow_score`에 저장하며, 코드 내부에서는 `momentum_score` alias도 함께 사용합니다. DB `scan_results`에는 `chart_score`, `technical_score`, `fundamental_score`, `theme_score`, `flow_score`, `composite_score`를 함께 저장합니다.

`fundamental_score`는 절대 재무 점수 55%와 업종 내 상대 점수 45%를 혼합합니다. 절대 점수는 기존 PER/PBR/ROE/매출성장률 기준을 유지하고, 상대 점수는 같은 섹터 내에서 PER/PBR은 낮을수록, ROE/매출성장률은 높을수록 좋게 평가합니다. 섹터별 유효 데이터가 10개 미만이면 50점 중립값 쪽으로 수축합니다.

`theme_score`는 섹터별 5일/20일 시장 대비 상대수익률, MA20 상회 종목 비율, 평균 거래량 비율, 20일 고점 돌파 비율, 평균 추세 점수를 조합합니다. 섹터 종목 수가 10개 미만이면 50점 중립값 쪽으로 수축해 소수 종목 섹터의 하루짜리 급등락 노이즈를 줄입니다.
...
- fundamentals 수집 기반: `price_to_book`, `return_on_equity`, `revenue_growth`, `market_cap`
