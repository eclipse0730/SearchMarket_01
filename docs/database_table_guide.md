...
| `theme_score` | 테마 점수 |
| `flow_score` | 가격/거래량 기반 모멘텀 점수. DB 호환용 컬럼명 |
...
| `fundamental_score` | 20% | 절대 PER/PBR/ROE/매출 성장률 55% + 업종 내 상대 점수 45% |
| `theme_score` | 15% | 섹터 5/20일 상대수익률, MA20 상회 비율, 거래량 유입, 20일 돌파 비율, 평균 추세 |
| `flow_score` | 10% | 모멘텀 점수. 거래량, 고점 대비 위치, 목표가 괴리, 등락률, 갭/캔들 |
...
`flow_score`는 실제 외국인/기관/개인 순매수 같은 주체별 수급 데이터가 아니라, 가격과 거래량으로 계산한 모멘텀 점수입니다. 스키마 호환성을 위해 컬럼명은 유지하지만, 코드와 리포트에서는 `momentum_score` 또는 가격 흐름 점수로 해석합니다.
