...
8. 금리, 달러, 원자재 등 매크로 시계열을 `daily_macro`에 저장
9. 시장/섹터 요약을 `market_snapshots`, `sector_snapshots`에 저장
10. 뉴스와 생성 리포트는 후속 테이블에 저장
...
|---|---|
| `run_id` | 실행 ID |
| `run_type` | `universe`, `prices`, `indicators`, `scan`, `news`, `render`, `backfill`, `fundamentals` |
| `market_key` | 실행 대상 시장 |
