...
|---|---|---|
| `kospi`, `kosdaq` | Naver 일봉 직접 조회만 사용 | `naver` |
| `us` | yfinance -> FinanceDataReader fallback | `yfinance` 또는 `fdr` |
| `global-indices`, `commodities` | yfinance | `yfinance` |

한국 시장은 timeout이 적용된 Naver 일봉 API만 사용합니다. 성공하면 `source_provider = 'naver'`로 저장되고, Naver 조회가 실패하거나 빈 결과이면 실패로 기록합니다.

US 시장은 yfinance를 먼저 사용하고, yfinance가 빈 결과를 반환하면 FinanceDataReader로 한 번 더 조회합니다.

현재 timeout은 2초입니다. 한국/Naver 경로에는 별도 retry/backoff가 없습니다.
...
| `source_provider` | 실제 성공 소스. 한국 시장은 `naver`, US는 `yfinance` 또는 `fdr`, 그 외 시장은 `yfinance`. 빈 결과는 저장하지 않음 |
...
```text
run_type = 'prices'
source_provider = 'naver'    # 한국 시장
source_provider = 'yfinance' # US 기본, 그 외 시장
source_provider = 'fdr'      # US yfinance fallback
