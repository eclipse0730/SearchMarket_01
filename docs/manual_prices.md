...
`Search.py scan <market>` 경로에서도 `market_scanner.pipeline.run_scan_stage_with_settings()`를 통해 `run_fetch()`가 먼저 호출됩니다.
...
| `--workers N` | 병렬 fetch worker 수. 기본 1, 최대 8 |
...
최근 실패한 `prices` 실행의 `error_samples`에 남은 전체 symbol을 다시 fetch합니다.
...
|---|---|---|
| `kospi`, `kosdaq` | FinanceDataReader | `fdr` |
| `us` | yfinance | `yfinance` |
| `global-indices`, `commodities` | yfinance | `yfinance` |

한국 시장은 FinanceDataReader로 일봉을 조회합니다. 성공하면 `source_provider = 'fdr'`로 저장되고, 조회가 실패하거나 빈 결과이면 실패로 기록합니다.

US 시장은 yfinance만 사용합니다. yfinance가 실패하거나 빈 결과를 반환하면 실패로 기록합니다.

현재 US/yfinance timeout은 2초입니다. 한국/FDR 경로에는 별도 retry/backoff가 없습니다.
...
12. 빈 결과나 예외는 `failed` 처리하고 `error_samples`에 전부 저장
13. 마지막에 `collection_runs`를 `success`, `partial`, `failed` 중 하나로 업데이트

출력 예:

```text
prices fetch [us] 120 symbols -> 2026-05-05  workers=1  run_id=...
...
`run_retry()`는 DB에서 실패 목록을 읽은 뒤 `run_fetch()`를 호출합니다.

조회 기준:

- `--run-id`가 있으면 해당 run의 `error_samples`
- 없으면 같은 시장에서 `run_type = 'prices'`이고 `status IN ('failed', 'partial')`인 가장 최근 실행

`error_samples` 안의 dict 중 `symbol` 키가 있는 항목을 재시도합니다. 재시도 범위는 각 항목의 `from`/`to` 날짜를 사용합니다.
...
| `source_provider` | 실제 성공 소스. 한국 시장은 `fdr`, US와 글로벌 지수/원자재는 `yfinance`. 빈 결과는 저장하지 않음 |
...
source_provider = 'yfinance' # US 기본, 그 외 시장
...
- 한국 시장의 성공 소스 라벨과 실제 조회 소스는 모두 `fdr`입니다.
