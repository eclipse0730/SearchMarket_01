...
- 한 번에 CSV를 제거하지 않습니다.
- DB를 먼저 쓰고, CSV는 DB에서 export할 수 있게 만듭니다.
- 과거 CSV는 필요 시 backfill 스크립트로 DB에 적재합니다.
- 소스별 실패는 `collection_runs`에 남기고, 실패 종목 샘플을 보관합니다.
- 가격 원천이 바뀌어도 지표 계산 결과를 재현할 수 있도록 `daily_indicators.price_source_provider`를 저장합니다.
- 시장/섹터 snapshot은 `universe_key`를 `NULL`로 두지 않습니다. 시장 전체를 뜻하는 경우에도 `kospi:all`, `us:all` 같은 명시 유니버스 키를 만들어 참조합니다.
- 현재 지표 스키마는 MA60/120/240을 컬럼으로 고정합니다. MA 기간을 동적으로 바꾸는 제품 요구가 생기면 long-form 지표 테이블로 전환합니다.
