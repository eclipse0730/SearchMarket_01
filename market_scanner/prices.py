...
def _fetch_fdr(symbol: str, start: str, end: str) -> pd.DataFrame:
    try:
...
def fetch_ohlcv(symbol: str, market_key: str, start: str, end: str) -> tuple[pd.DataFrame, str]:
    ...
    if _is_korea(market_key):
        hist = _fetch_fdr(symbol, start, end)
        if not hist.empty:
            return hist, "fdr"
...
def run_fetch(
    ...
    today = date.today() if not date_str else datetime.strptime(date_str, "%Y%m%d").date()
    end = today.isoformat()
    _, currency_code, _ = country_currency_for_market(market_key)
    primary_source = "fdr" if _is_korea(market_key) else "yfinance"
    ...
    with connect(explicit_url) as conn:
        ...
        if not instruments:
            print(f"  prices fetch [{market_key}]: no active instruments")
            return

        run_id = _create_prices_run(
            conn, market_key, "prices", today, len(instruments), primary_source,
            params={"mode": "incremental", "end": end},
        )
        print(f"  prices fetch [{market_key}] {len(instruments)} symbols → {end}  run_id={run_id}")
        ...
        for instr in instruments:
            ...
            last = _last_price_date(conn, instrument_id, primary_source)
...
def main() -> None:
    ...
    if args.command == "fetch":
        run_fetch(args.market, args.date, args.database_url, args.limit)
