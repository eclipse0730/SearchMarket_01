...
def _scope_for_page_key(page_key: str) -> tuple[str, str]:
    if page_key in {"nasdaq100", "sp500"}:
        return "us", page_key
    return home_market_key(page_key), page_key


def _latest_db_trade_date(market_key: str, universe_key: str) -> date | None:
    with connect() as conn:
        row = conn.execute(
            ...
            """,
            (market_key, universe_key, market_key, universe_key, market_key, universe_key),
        ).fetchone()
    return row[0] if row and row[0] else None


def _latest_db_frame(page_key: str) -> tuple[str, pd.DataFrame] | None:
    market_key, universe_key = _scope_for_page_key(page_key)
    trade_date = _latest_db_trade_date(market_key, universe_key)
    ...
    if frame.empty:
        return None
    return trade_date.strftime("%Y%m%d"), frame
...
def _build_market_page(market_key: str, title: str, description: str, slug: str) -> BuiltPage | None:
    db_frame = _latest_db_frame(market_key)
    if not db_frame:
        return None
    date_str, frame = db_frame
    if market_key in MARKETS:
        market = MARKETS[market_key]
    else:
        base_market_key, _ = _scope_for_page_key(market_key)
        market = replace(MARKETS[base_market_key], key=market_key, label=title, output_prefix=market_key)
    return _render_filtered_report(market, frame, slug, title, description, date_str)
...
def _load_us_source_page() -> BuiltPage | None:
    db_frame = _latest_db_frame("us")
    if not db_frame:
        return None
    date_str, frame = db_frame
    source_path = SITE_DIR / "_db" / "us"
    return BuiltPage("us", "us", "US Market", "", date_str, frame, source_path, source_path)
...
def build_site() -> list[BuiltPage]:
    ...
    us_source = _load_us_source_page()
    if missing_derived and us_source:
